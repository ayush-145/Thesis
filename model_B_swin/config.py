"""
config.py — Configuration for Model B: Swin-V2-Tiny
=====================================================
...
class PathConfig:
    ...
    output_dir: str = (
        ...
        else f"{COLAB_DRIVE_BASE}/outputs/model_B" if IS_COLAB
...
@dataclass
class TrainConfig:
    """APTOS-only fine-tuning from ImageNet — aggressive anti-overfitting."""
    epochs: int = 30
    lr: float = 5e-6               # Conservative LR for Swin on small data
    weight_decay: float = 0.05     # Strong regularization for transformer on small data
    optimizer: str = "AdamW"
    ...
    dropout: float = 0.2           # Increased dropout for small dataset
