...
def preprocess_fundus(
    ...
    """
    Modified preprocessing for Foundation Models.

    Pipeline:
        1. Auto-crop black borders
        2. Resize to target_size × target_size
        (Gaussian blur subtraction and CLAHE are disabled to avoid domain gap with pretrained features)

    Args:
        image: Raw BGR fundus image, uint8.
        target_size: Output resolution (e.g., 512 for CNN/Swin, 224 for RETFound).
        sigma_factor: Gaussian blur σ factor (UNUSED).
        clip_limit: CLAHE clip limit (UNUSED).
        tile_grid_size: CLAHE tile grid (UNUSED).
    ...
    # Step 3 & 4: (Disabled for RETFound to preserve raw image distribution)
