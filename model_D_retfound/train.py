"""
train.py — APTOS Fine-Tuning for ModelD_RETFound
==================================================
RETFound was pretrained on 1.6M retinal images via MAE.
Fine-tune on APTOS with 5-fold CV using LoRA adapters.
...
class CheckpointManager:
    ...
    def __init__(self, checkpoint_dir, model_name, fold):
...
def train_model_fold(fold, train_loader, val_loader, device):
    """Train a single fold: RETFound + LoRA → APTOS."""
    ...
    optimizer = torch.optim.AdamW(trainable_params, lr=lr,
        weight_decay=CFG.train.weight_decay, betas=CFG.train.betas, eps=CFG.train.eps)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        train_metrics = train_one_epoch(model, train_loader, criterion, optimizer, scaler, device,
            grad_accum_steps=CFG.data.grad_accum_steps, max_grad_norm=CFG.train.gradient_clip_max_norm,
        ...
        print(f"  Fold {fold+1} | Epoch {epoch+1:3d}/{epochs} | T_loss={train_metrics['loss']:.4f} "
              f"T_QWK={train_metrics['qwk']:.4f} | V_loss={val_metrics['loss']:.4f} "
              f"V_QWK={val_metrics['qwk']:.4f} | LR={current_lr:.2e} | {elapsed:.0f}s")
        ...
        if val_metrics["qwk"] > best_qwk + CFG.train.early_stop_min_delta:
            ...
            patience_counter += 1
        ckpt.save_state(model, optimizer, scaler, scheduler, epoch, best_qwk, patience_counter, history)
        if patience_counter >= CFG.train.early_stop_patience:
...
def run_single_model_pipeline(device: torch.device):
    """Train ModelD_RETFound: RETFound + LoRA → APTOS 5-fold CV."""
    model_name = MODEL_NAME
    img_size = get_img_size_for_model()
    pipeline_start = time.time()

    print("\n" + "=" * 60)
    print(f"  {model_name} — Fine-tuning on APTOS (RETFound + LoRA)")
    print("=" * 60)
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(folds):
        ...
        train_loader, val_loader = build_dataloaders(
            aptos_df, train_idx, val_idx, img_size,
            label_col="diagnosis")
